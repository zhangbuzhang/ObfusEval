...
def replace_function_and_compile(file_path, function_name, new_function_code,cwd):
    ...
    # 编译代码
    compile_command = ["cmake","-DFLB_DEV=On","../"]
    
    returncode, inform = exacutLinux(compile_command, build_dir)

    compile_command2 = ["make","-j8"]
...
if __name__ == '__main__':
    numOfInter = 5  
    testModelList = ['deepseek-coder']  # 'gpt-4-0125-preview' , 'gpt-4-1106-preview', 'gpt-3.5-turbo-0125'
    ...
    all_dict_df = pd.read_excel(root_path + "all_dict_se_fluent.xlsx",
    ...
    for testModel in testModelList:
        dataFile = root_path + "complete_symbol_input_fluent.xlsx"
        resultFile = root_path + testModel + "_complete_symbol_input_fluent_" + round + ".xlsx"
        ...
        for index, row in df_testSample.iterrows():
            ...
            for i in range(numOfInter):
                ...
                original_src_name = root_path + sourceCode_path+"fluent"
                new_src_name = root_path + sourceCode_path_copy + "fluent"
