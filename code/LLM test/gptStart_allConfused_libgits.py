...
def replace_c_function(file_path, target_function, new_function_code):
    ...
    if function_start is None or function_end is None:
        print(f"in file {file_path} not find {target_function}")
        return 0, covered
...
def replace_function_and_compile(file_path, function_name, new_function_code,cwd):
    ...
    print(f"compile success start test...")

    test_command = ["ctest","-E","^online$|^proxy$|^auth_clone$|^online_customcert","-V"]
...
def askChatGPT_with_retry(model, messages, max_retries=30, delay=30):
    ...
    for i in range(max_retries):
        try:
            ...
            print(f"request fail, try the {i + 1}/{max_retries}. error: {e}")
            time.sleep(delay)  
...
if __name__ == '__main__':
    numOfInter = 5  
    testModelList = ['gpt-3.5-turbo-0125']  # 'gpt-4-0125-preview' , 'gpt-4-1106-preview', 'gpt-3.5-turbo-0125'

    round = "v1"

    root_path = "/libgit2/libgit2/"

    sourceCode_path = "src"
    sourceCode_path_copy="src_copy"


    all_dict_df = pd.read_excel(root_path + "all_dict_sc_libgits.xlsx",
                                usecols=["FuncName", "FuncName_confused", "Used_Macros", "Used_Macros_confused",
                                         "Used_Global_Vars", "Used_Global_Vars_confused", "Used_Structs",
                                         "Used_Structs_confused"])

    for testModel in testModelList:
        dataFile = root_path + "complete_symbol_input_libgits.xlsx"
        resultFile = root_path + testModel + "_result_complete_symbol_input_libgits_" + round + ".xlsx"
        ...
        for index, row in df_testSample.iterrows():
            ...
            messages = [{"role": "system", "content": codeGenPrompt}]  # 重新开始会话
            ...
            for i in range(numOfInter):
                ...
                original_file_name = root_path + fileName
                ...
                response = replace_confuse_to_original(response, all_dict_df, "FuncName", "FuncName_confused")
                ...
                if state == 0:  
                    ...
                    error = replace_original_to_confuse(error, all_dict_df, "FuncName", "FuncName_confused")
