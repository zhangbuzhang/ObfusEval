...
def replace_function_and_compile(file_path, function_name, new_function_code,cwd):
    ...
    content_ninja = "/libvips-8.13.3/content_ninja.txt"
    current_directory1 = "/libvips-8.13.3/vips-8.13.3"
    current_directory2 = "/libvips-8.13.3/vips-8.13.3/build"

    print(f"file")
    state, covered = replace_c_function(file_path, function_name, new_function_code)

    if state == 0: return 0, "replace fail", None
    print(f"replace success start compile...")

    os.system("rm -rf " + current_directory2)
    os.mkdir(current_directory2)

    compile_command = ['meson','setup','build','--libdir=lib','--buildtype=release','-Dintrospection=false']
    
    returncode, inform = exacutLinux(compile_command, current_directory1)
    ...
    with open(content_ninja,"a") as file:
        try:
            result1 = subprocess.run(["ninja"],cwd=current_directory2, check=True,text=True,stdout=file, stderr=subprocess.STDOUT)
            if result1.returncode != 0: 
                if os.path.exists(content_ninja):
                    with open(content_ninja, 'r') as file:
                        temp_lines = file.readlines()
                        inform = '\n'.join(temp_lines)
                    os.remove(content_ninja)
                else:
                    inform ="not find" 
    ...
    #compile_command2 = ['ninja']
    #returncode1, inform1 = exacutLinux(compile_command2, current_directory2)
            if e.returncode != 0:  
                if os.path.exists(content_ninja):
                    with open(content_ninja, 'r') as file:
                        temp_lines = file.readlines()[-50:]
                        inform = '\n'.join(temp_lines)
                    os.remove(content_ninja)
                else:
                    inform ="not find" 
    ...
    returncode, inform = exacutLinux(test_command, current_directory2)
    if returncode != 0:  
...
if __name__ == '__main__':
    ...
    root_path = "/libvips-8.13.3/" 
    sourceCode_path = "vips-8.13.3/"
    sourceCode_path_copy="vips-8.13.3_copy/"


    dataFile = root_path + "complete_original_semantics_input_libvips.xlsx"
    resultFile = root_path + testModel + "_complete_original_semantics_input_libvips.xlsx"
    ...
    for index, row in df_testSample.iterrows():
        ...
        for i in range(numOfInter):
            ...
            original_src_name = root_path + sourceCode_path +"libvips/"
            new_src_name = root_path + sourceCode_path_copy+"libvips/"
